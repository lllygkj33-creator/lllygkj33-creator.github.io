...
- **内链一律用绝对路径**：`/docs/en/setup`、`/docs/en/sub-agents#enable-persistent-memory`（cli-reference 单页 136 条内链）。好处是复制到哪儿都不坏。本站在 markdown 里写 `/seo/keywords/xxx/`。
- **外链只给官方源**，商业链接带 utm（overview 里的 `claude.com/pricing?utm_source=claude_code&...`）。
- **收尾清单里每条链接后面跟一个冒号 + 一句话说明**，说明的是"点进去能得到什么"：
  ```markdown
